...
exports.getImagesSizes = function (files, options, callback) {
	...
	const imagePath = path.dirname(files[0].path);
	const command = `cd ${imagePath} && identify ` + filePaths.join(' ');
...
exports.generateImage = function (files, options, callback) {
	const imagePath = path.dirname(files[0].path);
	var command = [`cd ${imagePath} && convert -define png:exclude-chunks=date -quality 0% -size ` + options.width + 'x' + options.height + ' xc:none'];
	files.forEach(function (file) {
		command.push('"' + (file.name + file.extension) + '" -geometry +' + (file.x + options.padding) + '+' + (file.y + options.padding) + ' -composite');
	});
	command.push('"' + options.path + '/' + options.name + '.png"');
	const comm = command.join(' ');
	exec(comm, function (err) {
		if (err) return callback(err);
